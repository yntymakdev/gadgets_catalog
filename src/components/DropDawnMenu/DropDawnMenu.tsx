...
import { DEFAULT_PAGE, paginationOptions } from '../../constans/pagination';
...
export const DropdownMenu: React.FC<Props> = ({ items, size, params, defaultParams }) => {
  ...
  return (
    <div
      ...
      <div onClick={handleToogleDropdown} className={styles.Header}>
        ...
        {isOpen ? <img src={arrowUp} alt="up" /> : <img src={arrowDown} alt="down" />}
      </div>

      {isOpen && (
        <div className={styles.Items}>
          {items.map((item) => (
            <p key={item} onClick={() => handleSelectedItem(item)} className={styles.Item}>
              {item}
            </p>
          ))}
        </div>
